...
if [ $EXIST -ge 1 ]; then
	for i in *.$EXT; do
		...
		echo "% $COPYRIGHT"       			  > ../../$ARTIKEL/B-Keywords-$TEXNAME.tex # file anlegen
		echo "$T1A"               			 >> ../../$ARTIKEL/B-Keywords-$TEXNAME.tex
		echo "\newcommand{\thema}{$TEXNAME}" >> ../../$ARTIKEL/B-Keywords-$TEXNAME.tex
		echo "$T1B"               			 >> ../../$ARTIKEL/B-Keywords-$TEXNAME.tex
		echo "$T2"               			 >> ../../$ARTIKEL/B-Keywords-$TEXNAME.tex
		echo "	\input{$CONTENT/$TEX/$i}"    >> ../../$ARTIKEL/B-Keywords-$TEXNAME.tex
		echo "$T3"               			 >> ../../$ARTIKEL/B-Keywords-$TEXNAME.tex
	done
fi
cd ../..
